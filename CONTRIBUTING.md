...
Modules check the internal `BW_DEBUG` variable:
...
- Go 1.25+ is required to build the `bw status` or `bw analyze` TUI binaries locally.

## Go Components

`bw status` and `bw analyze` use Go with Bubble Tea for interactive dashboards.
