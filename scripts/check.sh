#!/bin/bash
# Code quality checks for Burrow.
...
echo -e "${BLUE}=== Burrow Check, ${MODE} ===${NC}\n"

SHELL_FILES=$(find . -type f \( -name "*.sh" -o -name "burrow" \) \
    -not -path "./.git/*" \
...
if [[ "$MODE" == "format" ]]; then
    ...
    if command -v goimports > /dev/null 2>&1; then
        ...
        goimports -w -local github.com/jimmy-jain/burrow ./cmd
...
if command -v shellcheck > /dev/null 2>&1; then
    if shellcheck burrow bin/*.sh lib/*/*.sh scripts/*.sh; then
...
if ! bash -n burrow; then
    echo -e "${RED}${ICON_ERROR} Syntax check failed, burrow${NC}\n"
    exit 1
fi
