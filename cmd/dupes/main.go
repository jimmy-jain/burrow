...
// Package main provides the mo dupes command for finding and managing duplicate files.
package main
...
func run() error {
	...
	fmt.Fprintf(os.Stderr, "mo dupes: scanning %s (min-size: %s)...\n",
...
func usage() {
	fmt.Fprintf(os.Stderr, `mo dupes — find and manage duplicate files

Usage:
  mo dupes [directory]              Report duplicates (default: current dir)
  mo dupes [directory] --json       Machine-readable output
  mo dupes [directory] --delete     Interactive deletion (Finder Trash)
  mo dupes [directory] --conserve <dir>  Move duplicates preserving paths
  mo dupes --restore <dir>          Restore conserved files
  mo dupes --restore <dir> --file <path>  Restore a single file
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintf(os.Stderr, "mo dupes: %v\n", err)
		os.Exit(1)
	}
}
