...
func TestIsHandledByMoClean(t *testing.T) {
	tests := []struct {
		...
		// Paths bw clean does NOT handle.
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := isHandledByBwClean(tt.path)
			if got != tt.want {
				t.Errorf("isHandledByBwClean(%q) = %v, want %v", tt.path, got, tt.want)
			}
		})
	}
}
...
func TestIsCleanableDir(t *testing.T) {
	tests := []struct {
		...
		// Paths handled by bw clean (should NOT be cleanable).
