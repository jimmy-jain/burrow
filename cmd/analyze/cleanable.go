...
// isCleanableDir marks paths safe to delete manually (not handled by bw clean).
func isCleanableDir(path string) bool {
	if path == "" {
		return false
	}

	// Exclude paths bw clean already handles.
	if isHandledByBwClean(path) {
		return false
	}
...
// isHandledByBwClean checks if a path is cleaned by bw clean.
func isHandledByBwClean(path string) bool {
	cleanPaths := []string{
		"/Library/Caches/",
		"/Library/Logs/",
