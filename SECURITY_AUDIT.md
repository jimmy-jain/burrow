# Burrow Security Audit

This document describes the security-relevant behavior of the current `main` branch. It is intended as a public description of Burrow's safety boundaries, destructive-operation controls, release integrity signals, and known limitations.

## Executive Summary

Burrow is a local system maintenance tool. Its main risk surface is not remote code execution; it is unintended local damage caused by cleanup, uninstall, optimize, purge, installer cleanup, or other destructive operations.
...
Burrow prioritizes bounded cleanup over aggressive cleanup. When uncertainty exists, the tool should refuse, skip, or require stronger confirmation instead of widening deletion scope.
...
The highest-risk areas in Burrow are:
...
`bw analyze` is intentionally lower-risk than cleanup flows:
...
Burrow has explicit protected-path and protected-category logic in addition to root-path blocking.
...
Burrow uses sudo for a subset of system-maintenance paths, but elevated behavior is still bounded by validation and protected-path rules.
...
When sudo is denied or unavailable, Burrow prefers skipping privileged cleanup to forcing execution through unsafe fallback behavior.

## Sensitive Data Exclusions

Burrow is not intended to aggressively delete high-value user data.
...
Burrow exposes multiple safety controls before and during destructive actions:
...
- operation logs are written to `~/.config/burrow/operations.log` unless disabled with `BW_NO_OPLOG=1`
...
Burrow treats release trust as part of its security posture, not just a packaging detail.
...
- `bw analyze` delete is safer because it uses Trash, but other cleanup flows are permanent once confirmed.
