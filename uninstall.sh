#!/bin/bash
# Mole - Standalone uninstaller.
# Works independently of the main mole installation, so it can clean up
# broken installs where "mo remove" fails due to missing libraries.
...
echo "Detecting Mole installations..."
...
if [[ -n "$brew_cmd" ]] && "$brew_cmd" list mole >/dev/null 2>&1; then
    is_homebrew=true
fi
...
for path in "/usr/local/bin/mole" "$HOME/.local/bin/mole" "/opt/local/bin/mole"; do
    if [[ -f "$path" ]]; then
...
if [[ -n "$found_mole" && -f "$found_mole" ]]; then
    if [[ ! -L "$found_mole" ]] || ! readlink "$found_mole" | grep -q "Cellar/mole"; then
        ...
        for existing in "${manual_installs[@]+"${manual_installs[@]}"}"; do
            [[ "$existing" == "$found_mole" ]] && local_exists=true
        done
        [[ "$local_exists" == "false" ]] && manual_installs+=("$found_mole")
    fi
fi

for path in "/usr/local/bin/mo" "$HOME/.local/bin/mo" "/opt/local/bin/mo"; do
    if [[ -f "$path" ]]; then
        if [[ ! -L "$path" ]] || ! readlink "$path" | grep -q "Cellar/mole"; then
            alias_installs+=("$path")
        fi
...
has_config=false
has_cache=false
[[ -d "$HOME/.config/mole" ]] && has_config=true
[[ -d "$HOME/.cache/mole" ]] && has_cache=true
...
[[ "$is_homebrew" == "true" ]] && echo -e "  ${ICON_LIST} Mole via Homebrew"
...
[[ "$has_config" == "true" ]] && echo -e "  ${ICON_LIST} ~/.config/mole"
[[ "$has_cache" == "true" ]] && echo -e "  ${ICON_LIST} ~/.cache/mole"
...
if [[ "$is_homebrew" == "true" ]]; then
    if [[ -n "$brew_cmd" ]]; then
        ...
        if ! "$brew_cmd" uninstall --force mole 2>&1; then
            has_error=true
            echo -e "${RED}Homebrew uninstall failed. Run manually: brew uninstall --force mole${NC}"
        else
...
maybe_sudo_rm_rf "$HOME/.cache/mole"
maybe_sudo_rm_rf "$HOME/.config/mole"

echo ""
if [[ "$has_error" == "true" ]]; then
    echo -e "${YELLOW}${ICON_ERROR} Mole uninstalled with some errors${NC}"
    exit 1
else
    echo -e "${GREEN}${ICON_SUCCESS} Mole uninstalled successfully${NC}"
fi
