#!/bin/bash
# Burrow - Installer for manual installs.
...
resolve_source_dir() {
    if [[ -n "$SOURCE_DIR" && -d "$SOURCE_DIR" && -f "$SOURCE_DIR/burrow" ]]; then
        return 0
    fi
    ...
    if [[ -n "${BASH_SOURCE[0]:-}" && -f "${BASH_SOURCE[0]}" ]]; then
        ...
        if [[ -f "$script_dir/burrow" ]]; then
            SOURCE_DIR="$script_dir"
            return 0
        fi
    fi

    if [[ -n "${CLEAN_SOURCE_DIR:-}" && -d "$CLEAN_SOURCE_DIR" && -f "$CLEAN_SOURCE_DIR/burrow" ]]; then
    ...
    local url="https://github.com/jimmy-jain/burrow/archive/refs/heads/main.tar.gz"

    if [[ "$branch" == "dev" ]]; then
        url="https://github.com/jimmy-jain/burrow/archive/refs/heads/dev.tar.gz"
    elif [[ "$branch" != "main" ]]; then
        url="https://github.com/jimmy-jain/burrow/archive/refs/tags/${branch}.tar.gz"
    fi

    start_line_spinner "Fetching Burrow source, ${branch}..."
    if command -v curl > /dev/null 2>&1; then
        if curl -fsSL --connect-timeout 10 --max-time 60 -o "$tmp/burrow.tar.gz" "$url" 2> /dev/null; then
            if tar -xzf "$tmp/burrow.tar.gz" -C "$tmp" 2> /dev/null; then
                stop_line_spinner

                local extracted_dir
                extracted_dir=$(find "$tmp" -mindepth 1 -maxdepth 1 -type d | head -n 1)

                if [[ -n "$extracted_dir" && -f "$extracted_dir/burrow" ]]; then
    ...
    start_line_spinner "Cloning Burrow source..."
    if command -v git > /dev/null 2>&1; then
        ...
        if git clone "${git_args[@]}" https://github.com/jimmy-jain/burrow.git "$tmp/burrow" > /dev/null 2>&1; then
            stop_line_spinner
            SOURCE_DIR="$tmp/burrow"
            return 0
        fi
    fi
    stop_line_spinner
...
# Version helpers
get_source_version() {
    local source_burrow="$SOURCE_DIR/burrow"
    if [[ -f "$source_burrow" ]]; then
        sed -n 's/^VERSION="\(.*\)"$/\1/p' "$source_burrow" | head -n1
    fi
}
...
get_source_commit_hash() {
    ...
    curl -fsSL --connect-timeout 3 \
        "https://api.github.com/repos/jimmy-jain/burrow/commits/main" 2> /dev/null |
...
get_latest_release_tag() {
    ...
    tag=$(curl -fsSL --connect-timeout 2 --max-time 3 \
        "https://api.github.com/repos/jimmy-jain/burrow/releases/latest" 2> /dev/null |
...
get_latest_release_tag_from_git() {
    ...
    git ls-remote --tags --refs https://github.com/jimmy-jain/burrow.git 2> /dev/null |
        awk -F/ '{print $NF}' |
        grep -E '^V[0-9]' |
        sort -V |
        tail -n 1
...
get_installed_version() {
    local binary="$INSTALL_DIR/burrow"
    if [[ -x "$binary" ]]; then
        local version
        version=$("$binary" --version 2> /dev/null | awk '/Burrow version/ {print $NF; exit}')
        if [[ -n "$version" ]]; then
            echo "$version"
        else
...
resolve_install_channel() {
    ...
    if [[ "${BURROW_EDGE_INSTALL:-}" == "true" ]]; then
        printf 'nightly\n'
        return 0
    fi
...
parse_args() {
    ...
    for i in "${!args[@]}"; do
        ...
        case "$token" in
            latest | main)
                export BURROW_VERSION="main"
                export BURROW_EDGE_INSTALL="true"
                version_token="$token"
                unset 'args[$i]'
                ;;
            dev)
                export BURROW_VERSION="dev"
                export BURROW_EDGE_INSTALL="true"
                version_token="$token"
                unset 'args[$i]'
                ;;
            [0-9]* | V[0-9]* | v[0-9]*)
                export BURROW_VERSION="$token"
                version_token="$token"
                unset 'args[$i]'
                ;;
            *)
...
check_requirements() {
    ...
    if command -v brew > /dev/null 2>&1 && brew list burrow > /dev/null 2>&1; then
        local burrow_path
        burrow_path=$(command -v burrow 2> /dev/null || true)
        local is_homebrew_binary=false

        if [[ -n "$burrow_path" && -L "$burrow_path" ]]; then
            if readlink "$burrow_path" | grep -q "Cellar/burrow"; then
        ...
        if [[ "$is_homebrew_binary" == "true" ]]; then
            ...
            echo -e "${YELLOW}Burrow is installed via Homebrew${NC}"
            echo ""
            echo "Choose one:"
            echo -e "  1. Update via Homebrew: ${GREEN}brew upgrade burrow${NC}"
            echo -e "  2. Switch to manual: ${GREEN}brew uninstall --force burrow${NC} then re-run this"
            echo ""
            exit 1
        else
            log_warning "Cleaning up stale Homebrew installation..."
            brew uninstall --force burrow > /dev/null 2>&1 || true
        fi
    fi
...
install_files() {
    ...
    if [[ -f "$SOURCE_DIR/burrow" ]]; then
        if [[ "$source_dir_abs" != "$install_dir_abs" ]]; then
            ...
            maybe_sudo mv -f "$INSTALL_DIR/burrow.new" "$INSTALL_DIR/burrow"

            log_success "Installed burrow to $INSTALL_DIR"
        fi
    else
        log_error "burrow executable not found in ${SOURCE_DIR:-unknown}"
        exit 1
    fi

    if [[ -f "$SOURCE_DIR/bw" ]]; then
        if [[ "$source_dir_abs" == "$install_dir_abs" ]]; then
            log_success "bw alias already present"
        else
            maybe_sudo cp "$SOURCE_DIR/bw" "$INSTALL_DIR/bw.new"
            maybe_sudo chmod +x "$INSTALL_DIR/bw.new"
            maybe_sudo mv -f "$INSTALL_DIR/bw.new" "$INSTALL_DIR/bw"
            log_success "Installed bw alias"
        fi
    fi
    ...
    # No sed patching needed: burrow resolves SCRIPT_DIR at runtime via
    # $BURROW_CONFIG_DIR or $HOME/.config/burrow, with fallback to script dir.
...
verify_installation() {
    ...
    if [[ -x "$INSTALL_DIR/burrow" ]] && [[ -f "$CONFIG_DIR/lib/core/common.sh" ]]; then

        if "$INSTALL_DIR/burrow" --help > /dev/null 2>&1; then
            return 0
        else
            log_warning "Burrow command installed but may not be working properly"
        fi
    else
...
setup_path() {
    ...
    if [[ "$INSTALL_DIR" != "/usr/local/bin" ]]; then
        ...
        echo ""
        echo "To use burrow from anywhere, add this line to your shell profile:"
...
print_usage_summary() {
    ...
    local message="Burrow ${action} successfully"
    ...
    echo "Usage:"
    if [[ ":$PATH:" == *":$INSTALL_DIR:"* ]]; then
        echo "  bw                           # Interactive menu"
        echo "  bw clean                     # Deep cleanup"
        echo "  bw uninstall                 # Remove apps + leftovers"
        echo "  bw optimize                  # Check and maintain system"
        echo "  bw analyze                   # Explore disk usage"
        echo "  bw status                    # Monitor system health"
        echo "  bw touchid                   # Configure Touch ID for sudo"
        echo "  bw update                    # Update to latest version"
        echo "  bw --help                    # Show all commands"
    else
        echo "  $INSTALL_DIR/bw                           # Interactive menu"
        echo "  $INSTALL_DIR/bw clean                     # Deep cleanup"
        echo "  $INSTALL_DIR/bw uninstall                 # Remove apps + leftovers"
        echo "  $INSTALL_DIR/bw optimize                  # Check and maintain system"
        echo "  $INSTALL_DIR/bw analyze                   # Explore disk usage"
        echo "  $INSTALL_DIR/bw status                    # Monitor system health"
        echo "  $INSTALL_DIR/bw touchid                   # Configure Touch ID for sudo"
        echo "  $INSTALL_DIR/bw update                    # Update to latest version"
        echo "  $INSTALL_DIR/bw --help                    # Show all commands"
    fi
    echo ""
}
...
perform_install() {
    ...
    if [[ "${BURROW_EDGE_INSTALL:-}" == "true" ]]; then
        ...
        echo ""
        local branch_name="${BURROW_VERSION:-main}"
        log_warning "Edge version installed on ${branch_name} branch"
        log_info "This is a testing version; use 'bw update' to switch to stable"
    fi
...
perform_update() {
    ...
    if command -v brew > /dev/null 2>&1 && brew list burrow > /dev/null 2>&1; then
        ...
        if [[ -f "$SOURCE_DIR/lib/core/common.sh" ]]; then
            ...
            log_error "Cannot update Homebrew-managed Burrow without full installation"
            echo ""
            echo "Please update via Homebrew:"
            echo -e "  ${GREEN}brew upgrade burrow${NC}"
            exit 1
        fi
        exit 0
    fi
    ...
    if [[ -z "$installed_version" ]]; then
        log_warning "Burrow is not currently installed in $INSTALL_DIR. Running fresh installation."
        perform_install
        return
    fi
    ...
    if [[ -z "$target_version" ]]; then
        log_error "Unable to determine the latest Burrow version."
        exit 1
    fi
