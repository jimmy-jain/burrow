...
_BURROW_MANAGE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$_BURROW_MANAGE_DIR/../core/common.sh"
source "$_BURROW_MANAGE_DIR/../ui/menu_simple.sh"

# Config file paths
readonly WHITELIST_CONFIG_CLEAN="$HOME/.config/burrow/whitelist"
readonly WHITELIST_CONFIG_OPTIMIZE="$HOME/.config/burrow/whitelist_optimize"
readonly WHITELIST_CONFIG_OPTIMIZE_LEGACY="$HOME/.config/burrow/whitelist_checks"
...
Burrow updates check|check_burrow_update|update_check
...
${GRAY}Edit: ${display_config}${NC}"
    ...
    if [[ ${#selected_cache_patterns[@]} -gt 0 ]]; then
        ...
        local IFS=','
        export BURROW_PRESELECTED_INDICES="${preselected_indices[*]}"
    else
        unset BURROW_PRESELECTED_INDICES
    fi

    BURROW_SELECTION_RESULT=""
    ...
    if [[ -n "$BURROW_SELECTION_RESULT" ]]; then
