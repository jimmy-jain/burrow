...
ask_for_updates() {
    ...
    if [[ -n "${BURROW_UPDATE_AVAILABLE:-}" && "${BURROW_UPDATE_AVAILABLE}" == "true" ]]; then
        has_updates=true
    fi
    ...
    if [[ "${BURROW_UPDATE_AVAILABLE:-}" == "true" ]]; then
        echo -ne "${YELLOW}Update Burrow now?${NC} ${GRAY}Enter confirm / ESC cancel${NC}: "
...
perform_updates() {
    # Only handle Burrow updates here; Homebrew/App Store/macOS are manual (tips shown in ask_for_updates)
    local updated_count=0
    local total_count=0

    if [[ -n "${BURROW_UPDATE_AVAILABLE:-}" && "${BURROW_UPDATE_AVAILABLE}" == "true" ]]; then
        echo -e "${BLUE}Updating Burrow...${NC}"
        local burrow_bin="${SCRIPT_DIR}/../../burrow"
        [[ ! -f "$burrow_bin" ]] && burrow_bin=$(command -v burrow 2> /dev/null || echo "")

        if [[ -x "$burrow_bin" ]]; then
            if "$burrow_bin" update 2>&1 | grep -qE "(Updated|latest version)"; then
                echo -e "${GREEN}${ICON_SUCCESS}${NC} Burrow updated"
                reset_burrow_cache
                updated_count=$((updated_count + 1))
            else
                echo -e "${RED}✗${NC} Burrow update failed"
            fi
        else
            echo -e "${RED}✗${NC} Burrow executable not found"
        fi
        echo ""
        total_count=1
    fi
