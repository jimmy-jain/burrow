...
if [[ -z "${PURGE_TARGETS:-}" ]]; then
    source "$_BURROW_MANAGE_DIR/../clean/project.sh"
fi

# Config file path (use :- to avoid re-declaration if already set)
PURGE_PATHS_CONFIG="${PURGE_PATHS_CONFIG:-$HOME/.config/burrow/purge_paths}"
...
# Burrow Purge Paths - Directories to scan for project artifacts
...
manage_purge_paths() {
    ...
    echo -e "${GRAY}Run 'bw purge' to clean with new paths${NC}"
    echo ""
}
