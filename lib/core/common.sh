#!/bin/bash
# Burrow - Common Functions Library
...
readonly BURROW_COMMON_LOADED=1
...
source "$_BURROW_CORE_DIR/file_ops.sh"
source "$_BURROW_CORE_DIR/help.sh"
source "$_BURROW_CORE_DIR/ui.sh"
source "$_BURROW_CORE_DIR/app_protection.sh"

# Load sudo management if available
if [[ -f "$_BURROW_CORE_DIR/sudo.sh" ]]; then
    source "$_BURROW_CORE_DIR/sudo.sh"
fi

# Normalize a path for comparisons while preserving root.
burrow_normalize_path() {
    local path="$1"
...
burrow_path_identity() {
    local path="$1"
    local normalized
    normalized=$(burrow_normalize_path "$path")
...
update_via_homebrew() {
    ...
    # Upgrade Burrow
    if [[ -t 1 ]]; then
        start_inline_spinner "Upgrading Burrow..."
    else
        echo "Upgrading Burrow..."
    fi

    brew upgrade burrow > "$temp_upgrade" 2>&1 &
    local upgrade_pid=$!
    ...
    if echo "$upgrade_output" | grep -q "already installed"; then
        ...
        installed_version=$(brew list --versions burrow 2> /dev/null | awk '{print $2}')
        [[ -z "$installed_version" ]] && installed_version=$(bw --version 2> /dev/null | awk '/Burrow version/ {print $3; exit}')
        echo ""
        ...
        local new_version
        new_version=$(brew list --versions burrow 2> /dev/null | awk '{print $2}')
        [[ -z "$new_version" ]] && new_version=$(bw --version 2> /dev/null | awk '/Burrow version/ {print $3; exit}')
        echo ""
    ...
    rm -f "$HOME/.cache/burrow/version_check" "$HOME/.cache/burrow/update_message" 2> /dev/null || true
}
