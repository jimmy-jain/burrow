#!/bin/bash
# Burrow - Logging System
# Centralized logging with rotation support

set -euo pipefail

# Prevent multiple sourcing
if [[ -n "${BURROW_LOG_LOADED:-}" ]]; then
    return 0
fi
readonly BURROW_LOG_LOADED=1
...
readonly LOG_FILE="${HOME}/Library/Logs/burrow/burrow.log"
readonly DEBUG_LOG_FILE="${HOME}/Library/Logs/burrow/burrow_debug_session.log"
readonly OPERATIONS_LOG_FILE="${HOME}/Library/Logs/burrow/operations.log"
...
rotate_log_once() {
    ...
    [[ -n "${BURROW_LOG_ROTATED:-}" ]] && return 0
    export BURROW_LOG_ROTATED=1
...
log_system_info() {
    ...
    [[ -n "${BURROW_SYS_INFO_LOGGED:-}" ]] && return 0
    export BURROW_SYS_INFO_LOGGED=1
    ...
    {
        echo "----------------------------------------------------------------------"
        echo "Burrow Debug Session, $(date '+%Y-%m-%d %H:%M:%S')"
        echo "----------------------------------------------------------------------"
        echo "User: $USER"
