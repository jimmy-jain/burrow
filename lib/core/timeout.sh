#!/bin/bash
# Burrow - Timeout Control
# Command execution with timeout support

set -euo pipefail

# Prevent multiple sourcing
if [[ -n "${BURROW_TIMEOUT_LOADED:-}" ]]; then
    return 0
fi
readonly BURROW_TIMEOUT_LOADED=1
...
# Sets BW_TIMEOUT_BIN to the available timeout command
#
...
if [[ -z "${BW_TIMEOUT_INITIALIZED:-}" ]]; then
    BW_TIMEOUT_BIN=""
    BW_TIMEOUT_PERL_BIN=""
    for candidate in gtimeout timeout; do
        if command -v "$candidate" > /dev/null 2>&1; then
            BW_TIMEOUT_BIN="$candidate"
    ...
    if command -v perl > /dev/null 2>&1; then
        BW_TIMEOUT_PERL_BIN="$(command -v perl)"
        if [[ -z "$BW_TIMEOUT_BIN" ]] && [[ "${BW_DEBUG:-0}" == "1" ]]; then
            echo "[TIMEOUT] Using perl fallback: $BW_TIMEOUT_PERL_BIN" >&2
        fi
    fi

    # Log warning if no timeout command available
    if [[ -z "$BW_TIMEOUT_BIN" && -z "$BW_TIMEOUT_PERL_BIN" ]] && [[ "${BW_DEBUG:-0}" == "1" ]]; then
    ...
    # Without this, children that inherit BW_TIMEOUT_INITIALIZED=1 skip the init
    # block but have empty bin vars, forcing the slow shell fallback.
    export BW_TIMEOUT_BIN
    export BW_TIMEOUT_PERL_BIN
    export BW_TIMEOUT_INITIALIZED=1
fi
...
#
# Environment:
#   BW_DEBUG - Set to 1 to enable debug logging to stderr
#
