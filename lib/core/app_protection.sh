#!/bin/bash
# Burrow - Application Protection
# System critical and data-protected application lists

set -euo pipefail

if [[ -n "${BURROW_APP_PROTECTION_LOADED:-}" ]]; then
    return 0
fi
readonly BURROW_APP_PROTECTION_LOADED=1

_BURROW_CORE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
[[ -z "${BURROW_BASE_LOADED:-}" ]] && source "$_BURROW_CORE_DIR/base.sh"
...
#
# In uninstall mode (BURROW_UNINSTALL_MODE=1), only system-critical components are protected.
...
should_protect_path() {
    ...
    if [[ "$path" =~ /Library/Containers/([^/]+) ]] || [[ "$path" =~ /Library/Group\ Containers/([^/]+) ]]; then
        ...
        if [[ "${BURROW_UNINSTALL_MODE:-0}" != "1" ]] && should_protect_data "$bundle_id"; then
            return 0
        fi
    fi
    ...
    # This catches things like /Users/user/Library/Caches/Claude when pattern is *Claude*
    # In uninstall mode, only check system-critical bundles (user explicitly chose to uninstall)
    if [[ "${BURROW_UNINSTALL_MODE:-0}" == "1" ]]; then
