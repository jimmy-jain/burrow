#!/bin/bash
# Burrow - UI Components
# Terminal UI utilities: cursor control, keyboard input, spinners, menus

set -euo pipefail

if [[ -n "${BURROW_UI_LOADED:-}" ]]; then
    return 0
fi
readonly BURROW_UI_LOADED=1
...
read_key() {
    ...
    if [[ "${BURROW_READ_KEY_FORCE_CHAR:-}" == "1" ]]; then
        [[ -z "$key" ]] && {
            echo "ENTER"
            return 0
        }
...
start_inline_spinner() {
    ...
    if [[ -t 1 ]]; then
        ...
        INLINE_SPINNER_STOP_FILE="${TMPDIR:-/tmp}/burrow_spinner_$$_$RANDOM.stop"
...
has_full_disk_access() {
    ...
    if [[ -n "${BURROW_HAS_FDA:-}" ]]; then
        if [[ "$BURROW_HAS_FDA" == "1" ]]; then
            return 0
        elif [[ "$BURROW_HAS_FDA" == "unknown" ]]; then
            return 2
        else
            return 1
        fi
