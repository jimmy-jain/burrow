...
select_apps_for_uninstall() {
    ...
    # - BURROW_MENU_META_EPOCHS: numeric last_used_epoch per item
    # - BURROW_MENU_META_SIZEKB: numeric size in KB per item
    # The menu will gracefully fallback if these are unset or malformed.
    if [[ $has_epoch_metadata == true ]]; then
        export BURROW_MENU_META_EPOCHS="$epochs_csv"
    else
        unset BURROW_MENU_META_EPOCHS
    fi
    if [[ $has_size_metadata == true ]]; then
        export BURROW_MENU_META_SIZEKB="$sizekb_csv"
    else
        unset BURROW_MENU_META_SIZEKB
    fi
    export BURROW_MENU_FILTER_NAMES="$names_newline"

    # Use paginated menu - result will be stored in BURROW_SELECTION_RESULT
    ...
    unset BURROW_MENU_META_EPOCHS BURROW_MENU_META_SIZEKB BURROW_MENU_FILTER_NAMES
    # leave BURROW_MENU_SORT_DEFAULT untouched if user set it globally
    ...
    IFS=',' read -r -a indices_array <<< "$BURROW_SELECTION_RESULT"
