...
readonly PURGE_TARGETS=("${BURROW_PURGE_TARGETS[@]}")
...
readonly DEFAULT_PURGE_SEARCH_PATHS=("${BURROW_PURGE_DEFAULT_SEARCH_PATHS[@]}")

# Config file for custom purge paths.
readonly PURGE_CONFIG_FILE="$HOME/.config/burrow/purge_paths"
...
readonly MONOREPO_INDICATORS=("${BURROW_PURGE_MONOREPO_INDICATORS[@]}")
readonly PROJECT_INDICATORS=("${BURROW_PURGE_PROJECT_INDICATORS[@]}")
...
# Burrow Purge Paths - Auto-discovered project directories
# Edit this file to customize, or run: bw purge --paths
...
load_purge_config() {
    ...
    done < <(burrow_purge_read_paths_config "$PURGE_CONFIG_FILE")
...
is_purge_project_root() {
    burrow_purge_is_project_root "$1"
}
...
scan_purge_targets() {
    ...
    local stats_dir="${XDG_CACHE_HOME:-$HOME/.cache}/burrow"
    ...
    # Allow forcing find via BW_USE_FIND environment variable
    if [[ "${BW_USE_FIND:-0}" == "1" ]]; then
        debug_log "BW_USE_FIND=1: Forcing find instead of fd"
        use_find=true
...
get_dir_size_kb() {
    ...
    local timeout_seconds="${BW_PURGE_SIZE_TIMEOUT_SEC:-15}"
...
clean_project_artifacts() {
    ...
    local dry_run_mode="${BURROW_DRY_RUN:-0}"
    for idx in "${selected_indices[@]}"; do
