...
burrow_hints_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck disable=SC1090
source "$burrow_hints_dir/purge_shared.sh"

# Quick reminder probe for project build artifacts handled by `bw purge`.
# Designed to be very fast: shallow directory checks only, no deep find scans.
# shellcheck disable=SC2329
load_quick_purge_hint_paths() {
    local config_file="$HOME/.config/burrow/purge_paths"
    local -a paths=()

    while IFS= read -r line; do
        [[ -n "$line" ]] && paths+=("$line")
    done < <(burrow_purge_read_paths_config "$config_file")

    if [[ ${#paths[@]} -eq 0 ]]; then
        paths=("${BURROW_PURGE_DEFAULT_SEARCH_PATHS[@]}")
    fi
...
probe_project_artifact_hints() {
    ...
    done < <(burrow_purge_quick_hint_target_names)
...
show_system_data_hint_notice() {
    ...
    echo -e "  ${GRAY}${ICON_REVIEW}${NC} Review: bw analyze, Device backups, docker system df"
}
...
show_project_artifact_hint_notice() {
    ...
    echo -e "  ${GRAY}${ICON_REVIEW}${NC} Review: bw purge"
}
