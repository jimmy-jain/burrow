...
if [[ -n "${BURROW_PURGE_SHARED_LOADED:-}" ]]; then
    return 0
fi
readonly BURROW_PURGE_SHARED_LOADED=1

# Canonical purge targets (heavy project build artifacts).
readonly BURROW_PURGE_TARGETS=(
    "node_modules"
    "target"        # Rust, Maven
...
readonly BURROW_PURGE_DEFAULT_SEARCH_PATHS=(
    "$HOME/www"
    "$HOME/dev"
    "$HOME/Projects"
    "$HOME/GitHub"
...
readonly BURROW_PURGE_MONOREPO_INDICATORS=(
    "lerna.json"
    "pnpm-workspace.yaml"
    "nx.json"
    "rush.json"
...
readonly BURROW_PURGE_PROJECT_INDICATORS=(
    "package.json"
    "Cargo.toml"
    "go.mod"
    "pyproject.toml"
...
# High-noise targets intentionally excluded from quick hint scans in bw clean.
readonly BURROW_PURGE_QUICK_HINT_EXCLUDED_TARGETS=(
    "bin"
    "vendor"
)

burrow_purge_is_project_root() {
    local dir="$1"
    local indicator

    for indicator in "${BURROW_PURGE_MONOREPO_INDICATORS[@]}"; do
        if [[ -e "$dir/$indicator" ]]; then
            return 0
        fi
    done

    for indicator in "${BURROW_PURGE_PROJECT_INDICATORS[@]}"; do
        if [[ -e "$dir/$indicator" ]]; then
            return 0
        fi
    done
...
burrow_purge_quick_hint_target_names() {
    local target
    local excluded
    local is_excluded

    for target in "${BURROW_PURGE_TARGETS[@]}"; do
        is_excluded=false
        for excluded in "${BURROW_PURGE_QUICK_HINT_EXCLUDED_TARGETS[@]}"; do
...
burrow_purge_read_paths_config() {
    local config_file="${1:-$HOME/.config/burrow/purge_paths}"
