...
_clean_mail_downloads() {
    local mail_age_days=${BURROW_MAIL_AGE_DAYS:-}
    ...
    for target_path in "${mail_dirs[@]}"; do
        if [[ -d "$target_path" ]]; then
            ...
            local min_kb="${BURROW_MAIL_DOWNLOADS_MIN_KB:-}"
            if ! [[ "$min_kb" =~ ^[0-9]+$ ]]; then
                min_kb=5120
            fi
...
clean_edge_old_versions() {
    ...
    local -a app_paths
    if [[ -n "${BURROW_EDGE_APP_PATHS:-}" ]]; then
        IFS=':' read -ra app_paths <<< "$BURROW_EDGE_APP_PATHS"
    else
        app_paths=(
...
clean_support_app_data() {
    local support_age_days="${BURROW_SUPPORT_CACHE_AGE_DAYS:-30}"
    ...
    if [[ "${BURROW_TEST_MODE:-0}" != "1" && "${BURROW_TEST_NO_AUTH:-0}" != "1" ]] && has_sudo_session; then
...
clean_application_support_logs() {
    ...
    local cleaned_count=0
    local found_any=false
    local size_timeout_seconds="${BURROW_APP_SUPPORT_ITEM_SIZE_TIMEOUT_SEC:-0.4}"
...
clean_cached_device_firmware() {
    ...
    if [[ "$cleaned_any" == "true" ]]; then
        ...
        if [[ "$DRY_RUN" == "true" ]]; then
            echo -e "  ${YELLOW}${ICON_DRY_RUN}${NC} Cached device firmware, ${YELLOW}${cleaned_count} files, $size_human dry${NC}"
        else
            echo -e "  ${GREEN}${ICON_SUCCESS}${NC} Cached device firmware, ${GREEN}${cleaned_count} files, $size_human${NC}"
        fi
        files_cleaned=$((files_cleaned + cleaned_count))
        total_size_cleaned=$((total_size_cleaned + total_size_kb))
        total_items=$((total_items + 1))
