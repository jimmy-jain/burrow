...
command_names=()
for entry in "${BURROW_COMMANDS[@]}"; do
    command_names+=("${entry%%:*}")
done
...
emit_fish_completions() {
    ...
    for entry in "${BURROW_COMMANDS[@]}"; do
        local name="${entry%%:*}"
        local desc="${entry#*:}"
        printf 'complete -c %s -n "__fish_burrow_no_subcommand" -a %s -d "%s"\n' "$cmd" "$name" "$desc"
    done

    printf '\n'
    printf 'complete -c %s -n "not __fish_burrow_no_subcommand" -a bash -d "generate bash completion" -n "__fish_see_subcommand_path completion"\n' "$cmd"
    printf 'complete -c %s -n "not __fish_burrow_no_subcommand" -a zsh -d "generate zsh completion" -n "__fish_see_subcommand_path completion"\n' "$cmd"
    printf 'complete -c %s -n "not __fish_burrow_no_subcommand" -a fish -d "generate fish completion" -n "__fish_see_subcommand_path completion"\n' "$cmd"
}
...
if [[ $# -eq 0 ]]; then
    ...
    case "$current_shell" in
        ...
        *)
            ...
            echo "  burrow completion <bash|zsh|fish>"
            exit 1
            ;;
    esac

    if [[ -z "$completion_name" ]]; then
        if [[ -f "$config_file" ]] && grep -Eq "(^# Burrow shell completion$|(burrow|bw)[[:space:]]+completion)" "$config_file" 2> /dev/null; then
            if [[ "${BURROW_DRY_RUN:-0}" == "1" ]]; then
                ...
                temp_file="$(mktemp)"
                grep -Ev "(^# Burrow shell completion$|(burrow|bw)[[:space:]]+completion)" "$config_file" > "$temp_file" || true
        ...
        log_error "burrow not found in PATH, install Burrow before enabling completion"
        exit 1
    fi

    # Check if already installed and normalize to latest line
    if [[ -f "$config_file" ]] && grep -Eq "(burrow|bw)[[:space:]]+completion" "$config_file" 2> /dev/null; then
        if [[ "${BURROW_DRY_RUN:-0}" == "1" ]]; then
    ...
    # Remove previous Burrow completion lines to avoid duplicates
    if [[ -f "$config_file" ]]; then
        original_mode=""
        original_mode="$(stat -f '%Mp%Lp' "$config_file" 2> /dev/null || true)"
        temp_file="$(mktemp)"
...
complete -F _burrow_completions burrow bw
EOF
        ;;
    zsh)
        printf '#compdef burrow bw\n\n'
        printf '_burrow() {\n'
        ...
        printf '}\n\n'
        printf 'compdef _burrow burrow bw\n'
        ;;
    fish)
        printf '# Completions for burrow\n'
        emit_fish_completions burrow
        printf '\n# Completions for bw (alias)\n'
        emit_fish_completions bw
        printf '\nfunction __fish_burrow_no_subcommand\n'
    ...
    *)
        cat << 'EOF'
Usage: burrow completion [bash|zsh|fish]

Setup shell tab completion for burrow and bw commands.

Auto-install:
  burrow completion              # Auto-detect shell and install
  burrow completion --dry-run    # Preview config changes without writing files

Manual install:
  burrow completion bash         # Generate bash completion script
  burrow completion zsh          # Generate zsh completion script
  burrow completion fish         # Generate fish completion script
...
Examples:
  ...
  # Manual install - Bash
  eval "$(burrow completion bash)"

  # Manual install - Zsh
  eval "$(burrow completion zsh)"

  # Manual install - Fish
  burrow completion fish | source
EOF
        exit 1
        ;;
esac
