...
EXPORT_LIST_FILE="$HOME/.config/burrow/clean-list.txt"
CURRENT_SECTION=""
...
WHITELIST_WARNINGS=()
if [[ -f "$HOME/.config/burrow/whitelist" ]]; then
    ...
    done < "$HOME/.config/burrow/whitelist"
else
...
register_dry_run_cleanup_target() {
    local path="$1"
    local identity
    identity=$(burrow_path_identity "$path")

    if [[ ${#DRY_RUN_SEEN_IDENTITIES[@]} -gt 0 ]] && burrow_identity_in_list "$identity" "${DRY_RUN_SEEN_IDENTITIES[@]}"; then
        return 1
    fi
...
safe_clean() {
    ...
    local permission_start=${BURROW_PERMISSION_DENIED_COUNT:-0}

    local show_scan_feedback=false
    if [[ ${#targets[@]} -gt 20 && -t 1 ]]; then
        show_scan_feedback=true
        stop_section_spinner
        BURROW_SPINNER_PREFIX="  " start_inline_spinner "Scanning ${#targets[@]} items..."
    fi
    ...
    if [[ "${BW_DEBUG:-}" == "1" && ${#existing_paths[@]} -gt 0 ]]; then
    ...
    if [[ ${#existing_paths[@]} -gt 10 ]]; then
        ...
        if [[ -t 1 ]]; then BURROW_SPINNER_PREFIX="  " start_inline_spinner "Scanning items..."; fi
    fi
    ...
    if [[ ${#existing_paths[@]} -gt 3 ]]; then
        ...
        if [[ $dir_count -lt 5 && ${#existing_paths[@]} -gt 20 ]]; then
            if [[ -t 1 && "$show_spinner" == "false" ]]; then
                BURROW_SPINNER_PREFIX="  " start_inline_spinner "Scanning items..."
                show_spinner=true
            fi
            ...
            if [[ ${#existing_paths[@]} -gt 0 ]]; then
                for path in "${existing_paths[@]}"; do
                    ...
                    if ((${#pids[@]} >= BURROW_MAX_PARALLEL_JOBS)); then
        ...
        if [[ "$DRY_RUN" != "true" && ${#existing_paths[@]} -gt 0 && -t 1 ]]; then
            BURROW_SPINNER_PREFIX="  " start_inline_spinner "Cleaning..."
    ...
    local permission_end=${BURROW_PERMISSION_DENIED_COUNT:-0}
...
start_cleanup() {
    ...
    export BURROW_CURRENT_COMMAND="clean"
...
# Burrow Cleanup Preview - $(date '+%Y-%m-%d %H:%M:%S')
#
# How to protect files:
# 1. Copy any path below to ~/.config/burrow/whitelist
# 2. Run: bw clean --whitelist
#
# Example:
...
EOF
        ...
        if has_sudo_session; then
            ...
            SYSTEM_CLEAN=false
            echo -e "${GRAY}${ICON_WARNING} System caches need sudo, run ${NC}sudo -v && bw clean --dry-run${GRAY} for full preview${NC}"
            echo ""
        fi
        return
    fi
...
ensure_recent_snapshot() {
    ...
    if [[ "${BW_SKIP_SNAPSHOT:-0}" == "1" ]]; then
        debug_log "Snapshot creation skipped (BW_SKIP_SNAPSHOT=1)"
        return 0
    fi
...
# shellcheck disable=SC2329
handle_first_run() {
    local sentinel_file="$HOME/.config/burrow/first_run_done"
