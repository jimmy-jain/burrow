...
PAM_SUDO_FILE="${BURROW_PAM_SUDO_FILE:-/etc/pam.d/sudo}"
PAM_SUDO_LOCAL_FILE="${BURROW_PAM_SUDO_LOCAL_FILE:-$(dirname "$PAM_SUDO_FILE")/sudo_local}"
...
enable_touchid() {
    ...
    if [[ ! -f "${PAM_SUDO_FILE}.burrow-backup" ]]; then
        if ! sudo cp "$PAM_SUDO_FILE" "${PAM_SUDO_FILE}.burrow-backup" 2> /dev/null; then
