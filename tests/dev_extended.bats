...
@test "clean_xcode_documentation_cache keeps newest DeveloperDocumentation index" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" BURROW_XCODE_DOCUMENTATION_CACHE_DIR="$doc_root" bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/dev.sh"
note_activity() { :; }
...
BURROW_JETBRAINS_TOOLBOX_KEEP=1
...
@test "clean_xcode_simulator_runtime_volumes shows scan progress and skips sizing in-use volumes" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" BURROW_XCODE_SIM_RUNTIME_VOLUMES_ROOT="$volumes_root" BURROW_XCODE_SIM_RUNTIME_CRYPTEX_ROOT="$cryptex_root" bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/dev.sh"
...
_sim_runtime_mount_points() {
    printf '%s\n' "$BURROW_XCODE_SIM_RUNTIME_VOLUMES_ROOT/in-use-runtime"
}
