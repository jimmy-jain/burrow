...
@test "dupes: conserve moves duplicates preserving paths" {
	...
	[ -f "$CONSERVE_DIR/.burrow-manifest.json" ]
}
...
@test "dupes: restore single file with --file" {
	...
	target="$(python3 -c "
import json
m = json.load(open('$CONSERVE_DIR/.burrow-manifest.json'))
...
@test "dupes: burrow dispatches to dupes" {
	run env HOME="$HOME" "$PROJECT_ROOT/burrow" dupes --help
	[ "$status" -eq 0 ]
	[[ "$output" == *"bw dupes"* ]]
}

@test "dupes: burrow help lists dupes command" {
