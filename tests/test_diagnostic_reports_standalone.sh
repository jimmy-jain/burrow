...
TMP_EMPTY=$(mktemp -d 2> /dev/null || mktemp -d -t burrow-test 2> /dev/null || echo "")
[[ -z "$TMP_EMPTY" ]] && TMP_EMPTY="/tmp/burrow-test-$$" && mkdir -p "$TMP_EMPTY"
...
TMP_DIAG=$(mktemp -d 2> /dev/null || mktemp -d -t burrow-diag 2> /dev/null || echo "/tmp/burrow-diag-$$")
TMP_APP=$(mktemp -d 2> /dev/null || mktemp -d -t burrow-app 2> /dev/null || echo "/tmp/burrow-app-$$")
...
TMP_DIAG2=$(mktemp -d 2> /dev/null || mktemp -d -t burrow-diag2 2> /dev/null || echo "/tmp/burrow-diag2-$$")
TMP_APP2=$(mktemp -d 2> /dev/null || mktemp -d -t burrow-app2 2> /dev/null || echo "/tmp/burrow-app2-$$")
