...
@test "BURROW_* constants are defined" {
    run bash -c "source '$PROJECT_ROOT/lib/core/common.sh'; echo \$BURROW_TEMP_FILE_AGE_DAYS"
    [ "$status" -eq 0 ]
    [ "$output" = "7" ]

    run bash -c "source '$PROJECT_ROOT/lib/core/common.sh'; echo \$BURROW_MAX_PARALLEL_JOBS"
    [ "$status" -eq 0 ]
    [ "$output" = "15" ]

    run bash -c "source '$PROJECT_ROOT/lib/core/common.sh'; echo \$BURROW_TM_BACKUP_SAFE_HOURS"
    [ "$status" -eq 0 ]
    [ "$output" = "48" ]
}
