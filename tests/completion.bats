...
@test "completion --help shows usage" {
	...
	[ "$status" -ne 0 ]
	[[ "$output" == *"Usage: burrow completion"* ]]
	[[ "$output" == *"Auto-install"* ]]
}
...
@test "completion bash generates valid bash script" {
	...
	[ "$status" -eq 0 ]
	[[ "$output" == *"_burrow_completions"* ]]
	[[ "$output" == *"complete -F _burrow_completions burrow bw"* ]]
}
...
@test "completion bash script supports bw command" {
	run "$PROJECT_ROOT/bin/completion.sh" bash
	[ "$status" -eq 0 ]
	[[ "$output" == *"complete -F _burrow_completions burrow bw"* ]]
}
...
@test "completion bash can be loaded in bash" {
	run bash -c "eval \"\$(\"$PROJECT_ROOT/bin/completion.sh\" bash)\" && complete -p burrow"
	[ "$status" -eq 0 ]
	[[ "$output" == *"_burrow_completions"* ]]
}
...
@test "completion fish generates valid fish script" {
	...
	[ "$status" -eq 0 ]
	[[ "$output" == *"complete -c burrow"* ]]
	[[ "$output" == *"complete -c bw"* ]]
}

@test "completion fish includes both burrow and bw commands" {
	output="$("$PROJECT_ROOT/bin/completion.sh" fish)"
	burrow_count=$(echo "$output" | grep -c "complete -c burrow")
	bw_count=$(echo "$output" | grep -c "complete -c bw")
...
@test "completion auto-install detects zsh" {
	...
	run grep -E "burrow[[:space:]]+completion" "$HOME/.zshrc"
	[ "$status" -eq 0 ]
}
...
@test "completion auto-install detects already installed" {
	...
	echo 'eval "$(burrow completion zsh)"' >"$HOME/.zshrc"
