...
@test "run_with_timeout: debug logging when BW_DEBUG=1" {
    output=$(bash -c "
        set -euo pipefail
        export BW_DEBUG=1
        source '$PROJECT_ROOT/lib/core/timeout.sh'
...
@test "run_with_timeout: no debug logging when BW_DEBUG=0" {
    output=$(bash -c "
        set -euo pipefail
        export BW_DEBUG=0
        unset BW_TIMEOUT_INITIALIZED
        source '$PROJECT_ROOT/lib/core/timeout.sh'
...
@test "timeout.sh: sets BURROW_TIMEOUT_LOADED flag" {
    result=$(bash -c "
        set -euo pipefail
        source '$PROJECT_ROOT/lib/core/timeout.sh'
        echo \"\$BURROW_TIMEOUT_LOADED\"
    ")
    [[ "$result" == "1" ]]
}
