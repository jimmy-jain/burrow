...
date +%s > "$HOME/.cache/burrow/brew_last_cleanup"
...
mkdir -p "$HOME/.cache/burrow"
rm -f "$HOME/.cache/burrow/brew_last_cleanup"
...
brew() { return 0; }
rm -f "$HOME/.cache/burrow/brew_updates"
...
source "$PROJECT_ROOT/lib/check/all.sh"
mkdir -p "$HOME/.cache/burrow"
cat > "$HOME/.cache/burrow/softwareupdate_list" <<'OUT'
Software Update Tool
...
rm -f "$HOME/.cache/burrow/softwareupdate_list"
...
@test "check_macos_update outputs debug info when BW_DEBUG set" {
    run bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/check/all.sh"
...
@test "run_with_timeout succeeds without GNU timeout" {
    run bash --noprofile --norc -c '
        set -euo pipefail
        PATH="/usr/bin:/bin"
        unset BW_TIMEOUT_INITIALIZED BW_TIMEOUT_BIN
        source "'"$PROJECT_ROOT"'/lib/core/common.sh"
...
@test "get_path_size_kb returns zero for missing directory" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" BW_DEBUG=0 bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
