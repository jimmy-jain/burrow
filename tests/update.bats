...
setup_file() {
    ...
    CURRENT_VERSION="$(grep '^VERSION=' "$PROJECT_ROOT/burrow" | head -1 | sed 's/VERSION=\"\\(.*\\)\"/\\1/')"
...
setup() {
    ...
    BURROW_UPDATE_AVAILABLE=false

    export MOCK_BIN_DIR="$BATS_TMPDIR/burrow-mocks-$$"
...
@test "perform_updates handles Homebrew success and Burrow update" {
    run bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/manage/update.sh"
...
cat > "$FAKE_DIR/burrow" <<'SCRIPT'
#!/usr/bin/env bash
echo "Already on latest version"
SCRIPT
chmod +x "$FAKE_DIR/burrow"
...
reset_burrow_cache() { echo "BURROW_CACHE_RESET"; }
...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ "$output" == *"Updating Burrow"* ]]
    [[ "$output" == *"Burrow updated"* ]]
    [[ "$output" == *"BURROW_CACHE_RESET"* ]]
...
set -euo pipefail
BURROW_TEST_BREW_UPDATE_OUTPUT="Updated 0 formulae"
BURROW_TEST_BREW_UPGRADE_OUTPUT="Warning: burrow 1.7.9 already installed"
BURROW_TEST_BREW_LIST_OUTPUT="burrow 1.7.9"
start_inline_spinner() { :; }
stop_inline_spinner() { :; }
brew() {
  case "$1" in
    update) echo "$BURROW_TEST_BREW_UPDATE_OUTPUT";;
    upgrade) echo "$BURROW_TEST_BREW_UPGRADE_OUTPUT";;
    list) if [[ "$2" == "--versions" ]]; then echo "$BURROW_TEST_BREW_LIST_OUTPUT"; fi ;;
  esac
}
...
@test "update_burrow skips download when already latest" {
...
process_install_output() {
    ...
    if ! printf '%s\n' "$output" | grep -Eq "Updated to latest version|Already on latest version"; then
        ...
        if [[ -z "$new_version" ]]; then
            new_version=$(command -v bw > /dev/null 2>&1 && bw --version 2> /dev/null | awk 'NR==1 && NF {print $NF}' || echo "")
        fi
        if [[ -z "$new_version" ]]; then
...
output="Installing Burrow...
◎ Burrow installed successfully, version 1.23.1"
...
@test "update_burrow with --force reinstalls even when on latest version" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" CURRENT_VERSION="$CURRENT_VERSION" PATH="$HOME/fake-bin:/usr/bin:/bin" TERM="dumb" bash --noprofile --norc << 'EOF'
set -euo pipefail
curl() {
  local out=""
  ...
  if [[ -n "$out" ]]; then
    cat > "$out" << 'INSTALLER'
#!/usr/bin/env bash
echo "Burrow installed successfully, version $CURRENT_VERSION"
INSTALLER
    return 0
  fi
...
"$PROJECT_ROOT/burrow" update --force
EOF
...
@test "update_burrow with --nightly uses installer path and passes BURROW_VERSION=main" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" CURRENT_VERSION="$CURRENT_VERSION" PATH="$HOME/fake-bin:/usr/bin:/bin" TERM="dumb" bash --noprofile --norc << 'EOF'
set -euo pipefail
curl() {
  local out=""
  ...
  if [[ -n "$out" ]]; then
    cat > "$out" << 'INSTALLER'
#!/usr/bin/env bash
echo "INSTALLER_BURROW_VERSION=${BURROW_VERSION:-}"
echo "Burrow installed successfully, version ${BURROW_VERSION:-unknown}"
INSTALLER
    return 0
  fi
...
"$PROJECT_ROOT/burrow" update --nightly
EOF

    [ "$status" -eq 0 ]
    [[ "$output" == *"Downloading nightly installer"* ]]
    [[ "$output" == *"Installing nightly update"* ]]
    [[ "$output" == *"INSTALLER_BURROW_VERSION=main"* ]]
    [[ "$output" == *"Updated to nightly build (main), main"* ]]
}

@test "update_burrow with --nightly is rejected for Homebrew installs" {
    local fake_brew_root="$HOME/fake-homebrew"
    local fake_cellar_bin="$fake_brew_root/Cellar/burrow/9.9.9/bin"
    local fake_path_bin="$HOME/fake-brew-bin"
    mkdir -p "$fake_cellar_bin" "$fake_path_bin"
    touch "$fake_cellar_bin/burrow"
    ln -sf "$fake_cellar_bin/burrow" "$fake_path_bin/burrow"
...
EOF
    ...
    [[ "$output" == *"bw update --nightly"* ]]
}

@test "get_homebrew_latest_version prefers brew outdated verbose target version" {
    run bash --noprofile --norc <<'EOF'
set -euo pipefail
BURROW_TEST_MODE=1 BURROW_SKIP_MAIN=1 source "$PROJECT_ROOT/burrow"

brew() {
  if [[ "${1:-}" == "outdated" ]]; then
    echo "jimmy-jain/burrow/burrow (1.29.0) < 1.30.0"
    return 0
  fi
  if [[ "${1:-}" == "info" ]]; then
    echo "==> jimmy-jain/burrow/burrow: stable 9.9.9 (bottled)"
    return 0
  fi
  return 0
}
...
brew() {
  ...
  if [[ "${1:-}" == "info" ]]; then
    echo "==> jimmy-jain/burrow/burrow: stable 1.31.1 (bottled), HEAD"
    return 0
  fi
  return 0
}
