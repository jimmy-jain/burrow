...
@test "remove_burrow deletes manual binaries and caches" {
	mkdir -p "$HOME/.local/bin"
	touch "$HOME/.local/bin/burrow"
	touch "$HOME/.local/bin/bw"
	mkdir -p "$HOME/.config/burrow" "$HOME/.cache/burrow"

	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PATH="/usr/bin:/bin" BURROW_TEST_MODE=1 bash --noprofile --norc <<'EOF'
set -euo pipefail
start_inline_spinner() { :; }
stop_inline_spinner() { :; }
rm() {
...
printf '\n' | "$PROJECT_ROOT/burrow" remove
EOF

	[ "$status" -eq 0 ]
	[ ! -f "$HOME/.local/bin/burrow" ]
	[ ! -f "$HOME/.local/bin/bw" ]
	[ ! -d "$HOME/.config/burrow" ]
	[ ! -d "$HOME/.cache/burrow" ]
}

@test "remove_burrow dry-run keeps manual binaries and caches" {
	mkdir -p "$HOME/.local/bin"
	touch "$HOME/.local/bin/burrow"
	touch "$HOME/.local/bin/bw"
	mkdir -p "$HOME/.config/burrow" "$HOME/.cache/burrow"
...
printf '\n' | "$PROJECT_ROOT/burrow" remove --dry-run
EOF
...
@test "remove_burrow test mode ignores PATH installs outside test HOME" {
	mkdir -p "$HOME/.local/bin" "$HOME/.config/burrow" "$HOME/.cache/burrow"
	touch "$HOME/.local/bin/burrow"
	touch "$HOME/.local/bin/bw"

	fake_global_bin="$(mktemp -d "${BATS_TEST_DIRNAME}/tmp-remove-path.XXXXXX")"
	touch "$fake_global_bin/burrow"
	touch "$fake_global_bin/bw"
...
EOF
	...
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PATH="$fake_global_bin:/usr/bin:/bin" BURROW_TEST_MODE=1 bash --noprofile --norc <<'EOF'
set -euo pipefail
start_inline_spinner() { :; }
stop_inline_spinner() { :; }
export -f start_inline_spinner stop_inline_spinner
...
EOF
	...
	[ "$status" -eq 0 ]
	[[ "$output" == *"$HOME/.local/bin/burrow"* ]]
	[[ "$output" == *"$HOME/.local/bin/bw"* ]]
	[[ "$output" != *"$fake_global_bin/burrow"* ]]
	[[ "$output" != *"$fake_global_bin/bw"* ]]
	[[ "$output" != *"brew uninstall --force burrow"* ]]
}
