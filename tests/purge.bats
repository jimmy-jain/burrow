...
@test "compact_purge_scan_path keeps the tail of long purge paths visible" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" BURROW_SKIP_MAIN=1 bash --noprofile --norc <<'EOF'
set -euo pipefail
...
grep -q "^~/" "$HOME/.config/burrow/purge_paths"
EOF
...
@test "scan_purge_targets: finds direct-child artifacts in project root with find mode" {
	...
	result=$(bash -c "
        source '$PROJECT_ROOT/lib/clean/project.sh'
        BW_USE_FIND=1 scan_purge_targets '$HOME/single-project' '$scan_output'
...
mkdir -p "$HOME/.cache/burrow"
echo "0" > "$HOME/.cache/burrow/purge_stats"
...
stats_dir="${XDG_CACHE_HOME:-$HOME/.cache}/burrow"
...
@test "bw purge: command exists and is executable" {
	[ -x "$PROJECT_ROOT/burrow" ]
	[ -f "$PROJECT_ROOT/bin/purge.sh" ]
}

@test "bw purge: shows in help text" {
	run env HOME="$HOME" "$PROJECT_ROOT/burrow" --help
	[ "$status" -eq 0 ]
	[[ "$output" == *"bw purge"* ]]
}

@test "bw purge: accepts --debug flag" {
	if ! command -v gtimeout >/dev/null 2>&1 && ! command -v timeout >/dev/null 2>&1; then
		skip "gtimeout/timeout not available"
	fi
	...
	run bash -c "
        export HOME='$HOME'
        $timeout_cmd 2 '$PROJECT_ROOT/burrow' purge --debug < /dev/null 2>&1 || true
    "
	true
}

@test "bw purge: accepts --dry-run flag" {
	if ! command -v gtimeout >/dev/null 2>&1 && ! command -v timeout >/dev/null 2>&1; then
		skip "gtimeout/timeout not available"
	fi
	...
	run bash -c "
        export HOME='$HOME'
        $timeout_cmd 2 '$PROJECT_ROOT/burrow' purge --dry-run < /dev/null 2>&1 || true
    "

	[[ "$output" == *"DRY RUN MODE"* ]] || [[ "$output" == *"Dry run complete"* ]]
}

@test "bw purge: creates cache directory for stats" {
	if ! command -v gtimeout >/dev/null 2>&1 && ! command -v timeout >/dev/null 2>&1; then
		skip "gtimeout/timeout not available"
	fi
	...
	bash -c "
        export HOME='$HOME'
        $timeout_cmd 2 '$PROJECT_ROOT/burrow' purge < /dev/null 2>&1 || true
    "

	[ -d "$HOME/.cache/burrow" ] || [ -d "${XDG_CACHE_HOME:-$HOME/.cache}/burrow" ]
}
