...
@test "check.sh script exists and is valid" {
    ...
    run bash -c "grep -q 'Burrow Check' '$PROJECT_ROOT/scripts/check.sh'"
    [ "$status" -eq 0 ]
}
...
@test "test.sh script exists and is valid" {
    ...
    run bash -c "grep -q 'Burrow Test Runner' '$PROJECT_ROOT/scripts/test.sh'"
    [ "$status" -eq 0 ]
}
...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ "$output" == *"Raycast: Burrow Clean | Alfred keyword: clean"* ]]
    [[ "$output" == *"Raycast: Burrow Status | Alfred keyword: status"* ]]

    local raycast_dir="$HOME/Library/Application Support/Raycast/script-commands"
    [ -d "$raycast_dir" ]

    local clean_script="$raycast_dir/burrow-clean.sh"
    local uninstall_script="$raycast_dir/burrow-uninstall.sh"
    local optimize_script="$raycast_dir/burrow-optimize.sh"
    local analyze_script="$raycast_dir/burrow-analyze.sh"
    local status_script="$raycast_dir/burrow-status.sh"
    ...
    run grep -q '^# @raycast.title Burrow Clean$' "$clean_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.title Burrow Uninstall$' "$uninstall_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.title Burrow Optimize$' "$optimize_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.title Burrow Analyze$' "$analyze_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.title Burrow Status$' "$status_script"
    [ "$status" -eq 0 ]

    run grep -q '^# @raycast.description Deep system cleanup with Burrow$' "$clean_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.description Uninstall applications with Burrow$' "$uninstall_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.description System health checks and optimization$' "$optimize_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.description Disk space analysis with Burrow$' "$analyze_script"
    [ "$status" -eq 0 ]
...
@test "install.sh supports dev branch installs" {
    ...
    [ "$status" -eq 0 ]
    run bash -c "grep -q 'BURROW_VERSION=\"dev\"' '$PROJECT_ROOT/install.sh'"
    [ "$status" -eq 0 ]
}
